...
LINT_PATHS = append_lint_paths(
    search_patterns=["django-polls-app"], lint_paths=LINT_PATHS
)
LINT_NOXFILES = ["noxfile.py", "official_tutorials/django-polls-app/noxfile.py"]
...
def run_linter(session: nox.Session):
    ...
    for _noxfile in LINT_NOXFILES:
        log.info(f"Linting {_noxfile}")
        session.run("black", _noxfile)
